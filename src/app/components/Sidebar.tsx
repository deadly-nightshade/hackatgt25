...
import type { Repository } from "./types"; // Chapter import not needed

type SidebarProps = {
  repositories: Repository[];
  currentRepo?: string;
  currentChapter?: string; // store chapter.id here
  isOpen?: boolean;
  onToggle?: () => void;
...
export default function Sidebar({
  ...
  const handleChapterClick = (repoId: string, chapterId: string) => {
    onNavigate?.(repoId, chapterId);
  };
    ...
    <>
      ...
      {isOpen && (
        <div
          ...
          onClick={() => onToggle?.()}   // wrap so handler matches MouseEvent signature
        />
      )}
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b-[3px] border-white bg-[linear-gradient(103deg,rgba(120,127,227,1)_0%,rgba(128,66,182,1)_100%)] flex-shrink-0">
          {!isCollapsed && <h2 className="text-lg font-bold text-white">Navigation</h2>}
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Mobile close button */}
            <button onClick={() => onToggle?.()} className="lg:hidden p-2 rounded-md hover:bg-white/20 transition-colors">
                ...
                <div key={repo.id} className="space-y-2">
                  ...
                  >
                    <div className={`flex items-center ${isCollapsed ? "justify-center" : ""}`}>
                      <svg
                        className={`flex-shrink-0 ${isCollapsed ? "w-5 h-5" : "w-4 h-4 mr-2"}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                  ...
                  {!isCollapsed && repo.chapters.length > 0 && (
                    <div className="ml-4 space-y-2">
                      {repo.chapters.map((chapter) => (
                        <button
                          key={chapter.id}
                          onClick={() => handleChapterClick(repo.id, chapter.id)}  // <-- use chapter.id
                          className={`
                            bg-none w-full text-left px-4 py-2 rounded-2xl text-sm transition-all duration-300 font-mono
                            ${
                              currentRepo === repo.id && currentChapter === chapter.id
          ...
          <div className={`text-xs text-[#491b72] font-mono font-bold ${isCollapsed ? "text-center" : ""}`}>
            {isCollapsed ? "v1.0" : (
              <>
                Git Good v1.0
                <br />
                Made with blood, sweat & tears by Archit, Kai Wen, Ishan, Sienna
              </>
            )}
          </div>
        </div>
