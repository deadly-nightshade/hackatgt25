...
import { GitHubRepoAnalyzer } from './github-analyzer';
...
async function main() {
  ...
  try {
    ...
    if (geminiApiKey) {
      ...
      const aiAnalyzer = new GeminiCodeAnalyzer(geminiApiKey);
    ...
    if (result.chapters) {
      ...
      if (result.chapters.chapters && Array.isArray(result.chapters.chapters)) {
        ...
        result.chapters.chapters.forEach((chapterContent: string, index: number) => {
