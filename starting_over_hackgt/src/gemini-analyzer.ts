...
export interface CodeAnalysis {
  [filename: string]: any; // The structure will match the JSON format defined in the prompt
}
...
export interface ChapterContent {
  chapters: string[]; // Array of Markdown content for each chapter
  error?: string;
  raw_response?: string;
}

export interface SingleChapterInput {
  chapterNum: number;
  ...
  chapterFilenames: Record<number, { num: number; name: string; filename: string }>;
  prevChapter: { num: number; name: string; filename: string } | null;
  nextChapter: { num: number; name: string; filename: string } | null;
...
export class GeminiCodeAnalyzer {
  ...
  private shouldAnalyzeFile(filename: string): boolean {
    ...
    const isSpecialFile = specialFiles.some(special => 
      filename.toLowerCase().includes(special.toLowerCase())
    );
   ...
   */
  private extractAnalyzableFiles(structure: FileNode[]): Array<{ path: string, content: string }> {
   ...
   */
  private createAnalysisPrompt(files: Array<{ path: string, content: string }>): string {
    let prompt = `
...
`;
   ...
   */
  async analyzeCodeStructure(structure: FileNode[]): Promise<CodeAnalysis> {
   ...
   */
  private async analyzeInChunks(files: Array<{ path: string, content: string }>): Promise<CodeAnalysis> {
   ...
   */
  private createAbstractionPrompt(files: Array<{ path: string, content: string }>, maxAbstractionNum: number = 7): string {
...
`;
   ...
   */
  async analyzeAbstractions(structure: FileNode[], maxAbstractionNum: number = 7): Promise<AbstractionAnalysis> {
   ...
   */
  private async processAbstractionPrompt(prompt: string): Promise<AbstractionAnalysis> {
    const result = await this.model.generateContent(prompt);
    const response = await result.response;
    const text = response.text();
   ...
   */
  private getContentForIndices(files: Array<{ path: string, content: string }>, indices: number[]): Record<string, string> {
  ...
  private createRelationshipPrompt(
    ...
    const relevantFilesContentMap = this.getContentForIndices(
      files, 
...
`;
  ...
  async analyzeRelationships(
    abstractionAnalysis: AbstractionAnalysis,
    structure: FileNode[]
    ...
    try {
      const prompt = this.createRelationshipPrompt(
        abstractionAnalysis.abstractions,
        analyzableFiles,
        "Repository"
      );
      ...
      if (prompt.length > maxPromptLength) {
        ...
        const simplifiedPrompt = this.createRelationshipPrompt(
          abstractionAnalysis.abstractions,
          limitedFiles,
          "Repository"
        );
   ...
   */
  private async processRelationshipPrompt(prompt: string, numAbstractions: number): Promise<RelationshipAnalysis> {
    const result = await this.model.generateContent(prompt);
    const response = await result.response;
    const text = response.text();
   ...
   */
  private createChapterOrderPrompt(
    abstractions: Abstraction[],
...
`;
  ...
  async orderChapters(
    ...
    try {
      const prompt = this.createChapterOrderPrompt(
        abstractionAnalysis.abstractions,
   ...
   */
  private processChapterOrderPrompt(text: string, numAbstractions: number): ChapterOrder {
   ...
   * Helper function to get content for specific file indices
   */
  private getContentForIndicesFromFiles(files: Array<{ path: string, content: string }>, indices: number[]): Record<string, string> {
   ...
   */
  private validateChapterContent(chapterContent: string, chapterNum: number, abstractionName: string): string {
   ...
   */
  private createSingleChapterPrompt(item: SingleChapterInput): string {
    const { 
      chapterNum, 
      abstractionDetails, 
      projectName, 
...
`;
  ...
  async writeChapters(
    ...
    const chapterItems = this.prepareChaptersData(
      chapterOrder.orderedIndices,
      abstractionAnalysis.abstractions,
      analyzableFiles,
      projectName
    );
    ...
    try {
      ...
      for (let i = 0; i < chapterItems.length; i++) {
        ...
        chapterContent = this.validateChapterContent(
          chapterContent,
          item.chapterNum,
          item.abstractionDetails.name
        );
